...
async function addPricingOnly() {
  console.log('💰 教室の料金情報を追加中...\n')
  ...
  try {
    ...
    for (const pricingInfo of pricingData) {
      // 対象教室を見つける
      const targetClassroom = classrooms.find(c => c.email === pricingInfo.email)
      if (!targetClassroom) {
        console.log(`⚠️ ${pricingInfo.email} の教室が見つかりません`)
        continue
      }
      ...
      const { error: updateError } = await supabase
        .from('classrooms')
        .update(pricingInfo.data)
        .eq('id', targetClassroom.id)

      if (updateError) {
        console.error(`❌ ${targetClassroom.name}の料金更新エラー:`, updateError)
      } else {
        console.log(`✅ ${targetClassroom.name} 料金情報更新完了`)
      }
    }
    ...
    const { data: finalClassrooms, error: finalError } = await supabase
      .from('classrooms')
      .select('name, tuition_info, trial_lesson_info')
      .eq('status', 'published')
